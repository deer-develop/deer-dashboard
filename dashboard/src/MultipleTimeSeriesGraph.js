...
export const color1 = '#4032DC'
export const color2 = '#94A3B8'
const tooltipStyles = {
	...defaultStyles,
	background,
	...
	({
		...
		const stockValueScale = useMemo(
			() =>
				scaleLinear({
					range: [innerHeight, 0],
					domain: [0, (max(maxData, getStockValue) || 0) + innerHeight / 3],
					nice: true,
				}),
			[0, innerHeight],
		)
		...
		const handleTooltip = useCallback(
			(event) => {
				...
				const index = bisectDate(data2, x0, 1)
				const d0 = data2[index - 1]
				const d1 = data2[index]
				let d = d0
				if (d1 && getDate(d1)) {
				...
				{tooltipData && (
					<div>
						<TooltipWithBounds
							...
							style={tooltipStyles}
						>
							<h4>{`${getStockValue(tooltipData)}${unit}`}</h4>
						</TooltipWithBounds>
						<Tooltip
