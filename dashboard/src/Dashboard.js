...
function Dashboard() {
	...
	const mondayFetcher = new Map([[coreUserProportionIndicatorsRefetch], [retentionCohortRefetch], [userCohortRefetch]])
	...
	return (
		<div>
			{status.some(isLoading) || npsDissatisfactionIndicators === undefined || retentionCohort === undefined ? (
					...
					<div style={{ display: 'flex', justifyContent: 'space-between', height: `${(BOX_1_H * 100) / (VIEWPORT_H - 70)}%` }}>
						<SectionBox style={{ display: 'flex', flexDirection: 'column' }} width={(BOX_11_W * 100) / (VIEWPORT_W - 70)}>
							<div style={{ width: '100%', height: `${(115 * 100) / BOX_1_H}%` }}>
								<MainIndicatorBox style={{ float: 'left', marginRight: '0px' }} color={'#5346E0'} width={(235 * 100) / BOX_11_W} height={100} image={'images/profile.png'} title={'핵심유저 비율'}>
									<h2 style={{ display: 'inline-block', marginTop: '23px', fontSize: '45px', color: '#ffffff' }}>{coreUserProportionIndicators.userProportion.toFixed(1)}</h2>
									<h3 style={{ display: 'inline-block', fontSize: '15px', color: '#ffffff', marginLeft: '7.25px' }}>%</h3>
									<DetailBox
										index={1}
										width={(100 * 100) / 235}
										height={(60 * 100) / 107}
										positiveImage={'images/plus_user.png'}
										negativeImage={'images/minus_user.png'}
										figure1={coreUserProportionIndicators.inflowUser.toLocaleString('ko-KR')}
										figure2={coreUserProportionIndicators.outflowUser.toLocaleString('ko-KR')}
										unit={'명'}
									></DetailBox>
								</MainIndicatorBox>
								<IndicatorBox style={{ display: 'flex', float: 'left', padding: '0px' }} width={(516 * 100) / BOX_11_W} height={(75 * 100) / 115}>
									...
									<hr style={{ border: 'none', position: 'absolute', left: '186px', margin: '0px', width: '0.6px', height: '100%', backgroundColor: '#CBD4E1' }}></hr>
							...
							<div style={{ width: '100%', paddingTop: '8px', height: `${(205 * 100) / BOX_1_H}%` }}>
								<div style={{ float: 'left', width: `${(320 * 100) / BOX_11_W}%`, height: '100%' }}>
						...
						<SectionBox style={{ display: 'flex', flexDirection: 'column' }} width={(BOX_12_W * 100) / (VIEWPORT_W - 70)}>
							...
							<div style={{ paddingTop: '18px', width: '100%', height: `${(215 * 100) / BOX_1_H}%` }}>
								...
								<div style={{ float: 'left', width: `${(364 * 100) / BOX_12_W}%`, height: '100%' }}>
									<div style={{ float: 'left', width: `${(240 * 100) / 364}`, height: '100%' }}>
									...
									<div style={{ float: 'left', margin: '15px 0px 0px 0px', width: `${(110 * 100) / 364}`, height: '100%' }}>{checkBoxOption}</div>
								</div>
							</div>
						</SectionBox>
						<SectionBox style={{ display: 'flex', flexDirection: 'column' }} width={(BOX_13_W * 100) / (VIEWPORT_W - 70)}>
							...
							<div style={{ width: '100%', height: `${(215 * 100) / BOX_1_H}%` }}>
								<div style={{ float: 'left', width: `${(240 * 100) / BOX_13_W}`, height: '100%' }}>
									<ProportionBarChart data={npsDissatisfactionIndicators.top6NpsDissatisfactionProportion} width={240} height={200} proportionMax={100} />
								</div>
					...
					<div style={{ display: 'flex', justifyContent: 'space-between', height: `${(BOX_2_H * 100) / (VIEWPORT_H - 70)}%` }}>
							...
							<div style={{ width: `${(420 * 100) / BOX_11_W}%`, height: '100%' }}>
								<div style={{ width: '100%', height: `${(90 * 100) / BOX_2_H}%` }}>
									<IndicatorBox style={{ display: 'flex', padding: '0px' }} width={(280 * 100) / 420} height={(75 * 100) / 90}>
										...
										<hr style={{ border: 'none', position: 'absolute', left: '140px', margin: '0px', width: '0.6px', height: '100%', backgroundColor: '#CBD4E1' }}></hr>
										<div style={{ boxSizing: 'border-box', padding: '13px 0px 0px 13px', width: '140px', height: '100%' }}>
											<h4>신규 가입자 수</h4>
						...
						<SectionBox style={{ display: 'flex', flexDirection: 'column' }} width={(BOX_13_W * 100) / (VIEWPORT_W - 70)}>
							...
							<div style={{ width: '100%', height: `${(125 * 100) / BOX_2_H}%` }}>
								<div style={{ float: 'left', width: `${(240 * 100) / BOX_13_W}`, height: '100%' }}></div>
								<div style={{ float: 'left', margin: '30px 0px 0px 8px', width: `${(70 * 100) / BOX_13_W}`, height: '100%' }}></div>
							</div>
						</SectionBox>
					</div>
					<div style={{ display: 'flex', justifyContent: 'space-between', height: `${(BOX_1_H * 100) / (VIEWPORT_H - 70)}%` }}>
							...
							<div style={{ width: '100%', height: `${(130 * 100) / BOX_1_H}%` }}>
								<div style={{ float: 'left', width: `${(400 * 100) / BOX_3_W}%`, height: '100%' }}>
									<MainIndicatorBox color={'#5346E0'} width={(385 * 100) / 400} height={(115 * 100) / 130} image={'images/coin.png'} title={'오늘 매출'}>
										<h2 style={{ display: 'inline-block', marginTop: '23px', fontSize: '45px', color: '#ffffff' }}>{revenueIndicators.revenue.toLocaleString('ko-KR')}</h2>
										<h3 style={{ display: 'inline-block', fontSize: '15px', color: '#ffffff', marginLeft: '7.25px' }}>만원</h3>
										<DetailBox index={2} style={{ bottom: '34px' }} width={(120 * 100) / 385} height={(30 * 100) / 115} positiveImage={'images/up_revenue.png'} negativeImage={'images/down_revenue.png'} figure1={revenueIndicators.revenueDayOnDay} unit={'만원'}></DetailBox>
									</MainIndicatorBox>
								</div>
						...
						<SectionBox width={(BOX_3_W * 100) / (VIEWPORT_W - 70)}>
							<MainIndicatorBox color={'#877CFF'} width={(328 * 100) / BOX_3_W} height={(115 * 100) / BOX_1_H} image={'images/wallet.png'} title={'회사 잔고'}>
								<DetailBox index={1} width={(110 * 100) / 328} height={(60 * 100) / 115} positiveImage={'images/plus_cash.png'} negativeImage={'images/minus_cash.png'}></DetailBox>
							</MainIndicatorBox>
						</SectionBox>
					</div>
				</Container>
