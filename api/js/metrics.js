...
async function getUserActivationHistoryMetric() {
    ...
    for (let i = 0; i < userActivationHistory.length; i++) {
        ...
        delete userActivationHistory[i]["install_app"]
        ["first_use", "install_app"].forEach(function (key) {
...
async function getNpsMetric(npsGroup) {
    const { promoters, passives, detractors, last_promoters, last_passives, last_detractors,
        zero, one, two, three, four, five, six, seven, eight, nine, ten,
        groupTotal, price, helmet, app, cs, hardware, forbidden, area, tatad, tooFast, tooSlow } = await repo.getNpsSectionIndicators(npsGroup)
    ...
    for (let i = 5; i >= 0; i--) {
        switch (npsDissatisfactionArray[i][0]) {
            case 'price': npsDissatisfactionArray[i][0] = '가격'; break;
            case 'helmet': npsDissatisfactionArray[i][0] = '헬멧'; break;
            case 'app': npsDissatisfactionArray[i][0] = '앱 사용'; break;
            case 'cs': npsDissatisfactionArray[i][0] = '고객 응대'; break;
            case 'hardware': npsDissatisfactionArray[i][0] = '킥보드 기기'; break;
            case 'forbidden': npsDissatisfactionArray[i][0] = '반납 금지 구역'; break;
            case 'area': npsDissatisfactionArray[i][0] = '이용 가능 지역'; break;
            case 'tatad': npsDissatisfactionArray[i][0] = '타고 싶을 때 없음'; break;
            case 'tooFast': npsDissatisfactionArray[i][0] = '속도 느림'; break;
            case 'tooSlow': npsDissatisfactionArray[i][0] = '속도 빠름'; break;
        }
...
async function getEarlyStageRetentionCohortMetric() {
    ...
    let retentionCohortArray = Array(9).fill(0).map((_, i) => Array(i + 1).fill(0))
    ...
    for (let i = 0; i < earlyStageRetentionCohort.length; i++) {
        let dateDiff = Math.ceil((now.getTime() - earlyStageRetentionCohort[i]["registered_date"].getTime()) / (1000 * 3600 * 24))
    ...
    for (let i = 0; i < 9; i++) {
        retentionCohortPercentage.push({
            ...
            bins: retentionCohortPercentageArray[i].map((percentage, idx) => ({ //retention_percentage
                //week_number: idx, value: percentage
                bin: idx, count: percentage
            })),
        })
    };
...
async function getUseDissatisfactionMetric() {
    ...
    const useDissatisfactionProportion = [
        {feedback: round((100 - (issueProportion + breakdownProportion)), 2), breakdown: breakdownProportion, issue: issueProportion}
    ]
...
async function getUseDissatisfactionDetailsMetric(type) {
    ...
    for (let i = 0; i <= 3; i++) {
        switch (useDissatisfactionArray[i][0]) {
            case '반납 X': useDissatisfactionArray[i][0] = '반납 안 됨'; break;
            case '파킹존 적립x': useDissatisfactionArray[i][0] = '파킹존 적립 안 됨'; break;
        }
...
async function getCoreRetentionCohortMetric() {
    ...
    for (let i = 0; i < coreRetentionCohort.length; i++) {
        let dateDiff = Math.ceil((now.getTime() - coreRetentionCohort[i]["registered_date"].getTime()) / (1000 * 3600 * 24))

        for (let j = 0; j < 9; j++) {
            if (coreRetentionCohort[i]["week_number"] === j) {
                let index = dateDiff / 7
